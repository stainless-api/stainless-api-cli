...
// resolveFilePaths returns the openapi spec path and stainless config path,
// preferring CLI flags over workspace config.
func resolveFilePaths(cmd *cli.Command, wc workspace.Config) (oasPath, configPath string) {
...
func runDevBuild(ctx context.Context, client stainless.Client, wc workspace.Config, cmd *cli.Command) error {
	...
	oasPath, configPath := resolveFilePaths(cmd, wc)
